...
export const CREATE_PRODUCTION_TOOL: Anthropic.Tool = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      data: {
        type: 'object',
        description: 'Production data. For report: { content: "<html>" }. For chart: { chartType, datasets[], labels[], options }. `options` accepts any Chart.js options object including `plugins.annotation.annotations` (chartjs-plugin-annotation is registered) for reference lines, thresholds, highlighted boxes, and labels — see the productions skill for the schema. For chronology: { title, entries: [{ sourceUrl, sourceLabel?, date, description, details?, highlight? }] }. sourceLabel is the short clickable text (e.g. "0x6ae5…"); if omitted, a label is auto-derived from sourceUrl. `highlight` is an optional row background color — one of "yellow" | "amber" | "red" | "green" | "blue" — used to call attention to a row in both the app and the export. Legacy `source` is also accepted as an alias for sourceUrl.',
      },
    },
...
- \`{ op: "chronology_set_row_highlight", indexes: [<int>, ...], color: "yellow" | "amber" | "red" | "green" | "blue" | null }\` — set or clear the background highlight on one or more chronology rows. Indexes are zero-based. \`color: null\` clears the highlight. Use this (not \`chronology_replace\`) when you only want to highlight rows — it's a fraction of the token cost and survives full row content. Semantics are up to you; reasonable defaults: \`red\` = suspicious/alert, \`amber\` = needs review, \`yellow\` = note, \`green\` = verified/cleared, \`blue\` = informational. Highlights render in both the in-app table and the PDF/HTML export.
