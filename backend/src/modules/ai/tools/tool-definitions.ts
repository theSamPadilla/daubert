...
- \`{ op: "chronology_replace", index: <int>, entry: {...} }\` — replace the entry at zero-based index. Use the index from the most recent \`read_production\`. If you sequenced multiple ops in one call, the index applies to the chronology AFTER prior ops in the array have been applied. Entry normalized inbound.
...
- \`{ op: "chronology_set_row_highlight", indexes: [<int>, ...], color: "yellow" | "gray" | "red" | "green" | "blue" | null }\` — color ∈ "yellow"|"gray"|"red"|"green"|"blue"|null. Set or clear the background highlight on one or more rows. Indexes are zero-based. Use this (not \`chronology_replace\`) when you only want to highlight rows.
- \`{ op: "chronology_set_column_widths", widths: { <columnKey>: 5–80, ... } }\` — set widths on any existing column key. Replaces the prior allowlist (source/date/description/details) — now accepts any key in data.columns. Each value is a number between 5 and 80 (percent of total table width). Partial updates are merged; missing columns keep their current width. The user normally sets these by dragging in the UI.
