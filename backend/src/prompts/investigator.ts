...
- append_chronology_entries: atomically append rows to a chronology without re-emitting existing rows. Strongly preferred over update_production when extending a chronology — saves output tokens proportional to chronology size.
...
- When extending an existing chronology with new rows, ALWAYS use append_chronology_entries — never update_production. Re-emitting the full chronology in update_production wastes tokens proportional to size and can hit max_tokens on long timelines.
