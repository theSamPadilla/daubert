...
- When asked to create a chronology, structure entries with 'source: { url, label? }' (explorer link + short display text, e.g. "0x6ae5…"; label is auto-derived if omitted), 'date', 'description', and optional 'details'. Legacy { sourceUrl, sourceLabel } is still accepted and normalized. Leave 'sourceTraceId'/'sourceEdgeId' for internal cross-references only. If the chronology has a dominant attribute (amounts, counterparties, exhibit numbers), add a custom column for it via 'chronology_add_column' so values are first-class — load the productions skill for column op details.
- When asked to flag, highlight, or call attention to specific chronology rows, use the 'chronology_set_row_highlight' op (colors: yellow|gray|red|green|blue, or null to clear). Don't rewrite the row with chronology_replace just to add a highlight.
- When creating or modifying a large chronology, never pass the full 'data' blob — it counts against the per-turn output cap and routinely hits max_tokens. For creation, seed an empty chronology with create_production ({ name, data: { entries: [] } }), then loop update_production with the 'chronology_append' op, ~50 entries per call. For edits, use 'ops' for the specific change (append rows, replace one row, delete rows, add/remove columns) so the cost is proportional to the change, not the chronology length.
- When asked to create a chart, use Chart.js-compatible data: { chartType: "bar"|"line"|"pie"|"doughnut", labels: [...], datasets: [{ label, data }], options? }. For reference lines, thresholds, event markers, or highlighted regions, use chartjs-plugin-annotation via options.plugins.annotation.annotations (supports types: line, box, point, label, ellipse). See the productions skill for the annotation schema.
