...
- [ ] Public `/external/trace` widget + website Solana support. The widget DTO (`backend/src/modules/external-trace/dto/trace-query.dto.ts`) is intentionally frozen at 6 chains (EVM ×4, Tron, Bitcoin) — Solana was deliberately left out of this ship, same rollout order Bitcoin followed. Needs its own cross-repo plan, mirroring `../website-daubert/docs/plans/2026-08-05-btc-widget.md`.
