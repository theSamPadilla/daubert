...
Both env keys are validated at boot (`backend/src/config/env.validation.ts`) — the app refuses to start without them. **There is deliberately no `BITCOIN_API_KEY`**: Bitcoin runs entirely on public, keyless Esplora endpoints.
...
Because chain config is consolidated, a new account-model chain is: one entry in `shared/chains.ts`, an address-family branch in `shared/address.ts` (if it's a new format), a provider (or an existing one if Etherscan V2 covers it), `npm run gen`, an icon in `ChainSelect`, sandbox domains + skill doc for the AI. A new UTXO chain additionally reuses the `UtxoProvider` seam. The public `/external/trace` widget's chain list (`SUPPORTED_CHAINS` in `trace-query.dto.ts`) is maintained independently of `shared/chains.ts` — it no longer excludes Bitcoin (added 2026-08), but extending it further is still always a deliberate, separate decision, not something `npm run gen` picks up automatically.
