  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <span className="text-sm font-semibold text-canvas-ink">{count} nodes selected</span>
      ...
      {/* Rename All */}
      <div>
        <h4 className="text-xs font-semibold text-canvas-muted uppercase mb-2">Rename All</h4>
        <div className="flex gap-2">
          <input
            ...
            placeholder="Prefix..."
            className="flex-1 bg-canvas-fill border border-canvas-line rounded-lg px-2 py-1 text-sm text-canvas-ink placeholder:text-canvas-muted focus:outline-none focus-visible:ring-2 focus-visible:ring-brand/40"
          />
          <button
            onClick={() => { if (prefix.trim()) { onRename(prefix.trim()); setPrefix(''); } }}
            disabled={!prefix.trim()}
            className="px-3 py-1 bg-brand text-white hover:bg-brand-strong disabled:opacity-40 disabled:hover:bg-brand rounded-lg text-xs"
          >
            Apply
          </button>
        </div>
        <p className="text-[10px] text-canvas-muted mt-1">Generates &quot;Prefix 1&quot;, &quot;Prefix 2&quot;, etc.</p>
      </div>

      {/* Change Color */}
      <div>
        <h4 className="text-xs font-semibold text-canvas-muted uppercase mb-2">Change Color</h4>
      ...
      {(onGroupNodes || onExtractToTrace || onAddToGroup) && (
        <div className="pt-3 border-t border-canvas-line space-y-2">
            ...
            <div className="rounded-lg border border-violet-500/30 bg-violet-500/10 p-3 space-y-2">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-canvas-fill border border-violet-500/40 rounded-lg px-2 py-1 text-sm text-canvas-ink placeholder:text-canvas-muted focus:outline-none focus:border-violet-400"
                />
                <button
        ...
        {confirmDelete ? (
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="px-2 py-1 text-xs text-canvas-muted hover:text-canvas-ink"
            >
              Cancel
            </button>
          </div>
