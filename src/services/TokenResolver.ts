...
// Well-known tokens per chain (address → metadata)
...
export class TokenResolver {
  ...
  async resolveAsync(chain: string, tokenAddress: string, provider: BlockchainProvider): Promise<TokenMetadata | null> {
    const immediate = this.resolve(chain, tokenAddress);
    if (immediate) return immediate;

    const result = await provider.getTokenMetadata(tokenAddress);
    if (result) {
      this.memoryCache.set(`${chain}:${tokenAddress.toLowerCase()}`, result);
    }
    return result;
  }

  resolveFromTransfer(chain: string, contractAddress: string, symbol: string, decimals: number): TokenMetadata {
